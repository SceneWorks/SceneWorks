...
The Stability AI Stable Diffusion 3.5 family — multimodal diffusion transformer (MMDiT) models running through native MLX on Apple Silicon and Candle/CUDA on Windows/Linux. All three variants share a triple text encoder (CLIP-L + CLIP-G + T5-XXL), which gives SD3.5 strong prompt adherence, reliable composition, and clean in-image text:
...
> **License:** Stability AI Community License. SceneWorks provides a public, ungated re-host (Powered by Stability AI), so download does not require a Hugging Face token or separate license acceptance. Free for non-commercial use and for commercial use by organizations under $1M annual revenue (see the license for terms).

> **Hardware:** MLX uses the prebuilt q4/q8/bf16 tiers on Apple Silicon; Candle/CUDA uses the corresponding native tiers on Windows/Linux. The triple text encoder (including the large T5-XXL) dominates the footprint, so Medium is not dramatically lighter than Large at load.
