...
export function VideoStudio({
  ...
  useEffect(() => {
    ...
    const replacementModel = videoModels.find((item) => item.capabilities?.includes("replace_person"));
    if (replacementModel) {
  ...
  const comparisonAsset = latestAssets.find((asset) => asset.recipe?.mode === "replace_person") ?? latestAssets[0];
  ...
  function analyzeSource() {
    ...
    createPersonDetectionJob({ sourceAssetId: sourceClipAssetId });
  }
    ...
    <section className="main-surface video-studio">
      ...
      <form className="studio-layout video-layout" onSubmit={submit}>
        <section className="studio-controls">
          ...
          {mode === "extend_clip" || mode === "replace_person" ? (
            <label>
              Source clip
