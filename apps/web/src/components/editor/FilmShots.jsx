        ...
        {shot ? (
          <article className="ve-film-shot-inspector">
            ...
            <fieldset className="ve-film-inspector-fields" disabled={disabled}>
              ...
              <label>Shot ID<input aria-label="Shot ID" value={shot.id} onChange={(event) => { const before = shot.id; const after = event.target.value; mutateShot((next) => { next.id = after; }); setSelectedShotIds((ids) => ids.map((id) => id === before ? after : id)); }} /></label>
