...
class PuLIDPipeline(nn.Module):
    def __init__(self, dit, device, weight_dtype=torch.bfloat16, onnx_provider='gpu', *args, **kwargs):
        ...
        # antelopev2
        # PATCH (SceneWorks): take the antelopev2 root from PULID_FLUX_INSIGHTFACE_ROOT
        # (the worker pre-provisions the 5 onnx files at $root/models/antelopev2 via
        # the shared sc-2009 InstantID helper). The upstream snapshot_download into
        # models/antelopev2 alongside cwd is dropped — the worker would otherwise need
        # a writable models/ next to wherever it was launched from.
        providers = ['CPUExecutionProvider'] if onnx_provider == 'cpu' \
            else ['CUDAExecutionProvider', 'CPUExecutionProvider']
        antelope_root = os.environ.get('PULID_FLUX_INSIGHTFACE_ROOT', str(Path.home() / '.insightface'))
        self.app = FaceAnalysis(name='antelopev2', root=antelope_root, providers=providers)
        self.app.prepare(ctx_id=0, det_size=(640, 640))
        glintr_path = str(Path(antelope_root) / 'models' / 'antelopev2' / 'glintr100.onnx')
        self.handler_ante = insightface.model_zoo.get_model(glintr_path, providers=providers)
    ...
    def load_pretrain(self, pretrain_path=None, version='v0.9.0'):
        # PATCH (SceneWorks): drop local_dir='models' — return the cache path directly.
        if pretrain_path is None:
            pretrain_path = hf_hub_download('guozinan/PuLID', f'pulid_flux_{version}.safetensors')
