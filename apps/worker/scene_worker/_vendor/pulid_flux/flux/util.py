...
def load_flow_model(name: str, device: str = "cuda", hf_download: bool = True):
    ...
    if (
        ...
        # PATCH (SceneWorks): drop local_dir='models' — return the shared HF cache path
        # directly so the worker process never depends on a writable models/ alongside cwd.
...
def load_ae(name: str, device: str = "cuda", hf_download: bool = True) -> AutoEncoder:
    ...
    if (
        ...
        # PATCH (SceneWorks): drop local_dir='models' for the same reason as load_flow_model.
