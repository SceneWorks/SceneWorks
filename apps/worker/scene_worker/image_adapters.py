...
def create_image_adapter(
    ...
    if model_target.get("adapter") == ZImageDiffusersAdapter.id:
        ...
        # Rust `mlx` GPU worker via the API routing layer (epic 3018 sc-3022/3028/3146).
        # The Python worker only sees jobs the mlx worker didn't claim (Windows/Linux, or
        # Mac when the mlx worker is down), so it routes the torch path. sc-3032.
        return adapters.get("z_image_diffusers") if adapters else ZImageDiffusersAdapter()
    if model_target.get("adapter") == QwenImageAdapter.id:
        # Qwen-Image on Mac is claimed by the Rust `mlx` GPU worker (epic 3018 sc-3024);
    ...
    if model_target.get("adapter") == FluxDiffusersAdapter.id:
        # FLUX.1 (schnell/dev) on Mac is claimed by the Rust `mlx` GPU worker (epic 3018
        # sc-3023); the Python worker routes the torch fallback (Windows/Linux, or Mac
        # when the mlx worker is down). sc-3032.
        return adapters.get("flux_diffusers") if adapters else FluxDiffusersAdapter()
    if model_target.get("adapter") == KolorsDiffusersAdapter.id:
        return adapters.get("kolors_diffusers") if adapters else KolorsDiffusersAdapter()
    if model_target.get("adapter") == "mlx_flux2":
        # FLUX.2-klein is MLX-only and has no torch fallback. After the epic 3018 cutover
        # (sc-3032) the only generation path is the Rust `mlx` GPU worker (mlx-gen-flux2),
        # which claims these jobs via the API routing layer. The frontend hides the model
        # on non-Mac hosts via the manifest `macOnly` flag; if a FLUX.2-klein job ever
        # reaches the Python worker (e.g. the mlx worker is down), fail loudly rather than
        # silently rendering the procedural placeholder.
        raise RuntimeError(
