...
const INLINE_RULES = [
  { re: /`([^`]+)`/, node: (m, key) => <code key={key}>{m[1]}</code> },
  {
    ...
    node: (m, key) => {
      ...
      return <a key={key} href={href} target="_blank" rel="noopener noreferrer">{renderInline(m[1], key)}</a>;
    },
  },
...
function parseBlocks(content) {
  ...
  while (i < lines.length) {
    ...
    if (FENCE.test(line.trim())) {
      i += 1;
      const code = [];
      while (i < lines.length && !FENCE.test(lines[i].trim())) { code.push(lines[i]); i += 1; }
      i += 1;
    ...
    if (LIST_ITEM.test(line)) {
      ...
      const items = [];
      while (i < lines.length && LIST_ITEM.test(lines[i])) { items.push(lines[i].replace(LIST_ITEM, "")); i += 1; }
      blocks.push({ type: "list", ordered, items });
      continue;
    }
    if (QUOTE.test(line)) {
      const quote = [];
      while (i < lines.length && QUOTE.test(lines[i])) { quote.push(lines[i].replace(QUOTE, "")); i += 1; }
...
function renderBlock(block, key) {
  switch (block.type) {
    ...
    case "list": {
      ...
      return <Tag key={key}>{block.items.map((item, index) => <li key={`${key}-${index}`}>{renderInline(item, `${key}-${index}`)}</li>)}</Tag>;
    }
    case "quote":
      return <blockquote key={key}><p>{renderInline(block.text, key)}</p></blockquote>;
    case "code":
