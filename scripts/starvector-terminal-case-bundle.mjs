...
export async function materializeBundle({ corpusPath, assetsRoot, output, permanentPin, bindingPath, hostileGenerator = null }) {
  ...
  for (const [position, row] of index.rows.entries()) {
    if (row.case_index !== position || !row.filename || !/^[a-f0-9]{64}$/.test(row.svg_sha256) || !/^[a-f0-9]{64}$/.test(row.png_sha256) || !/^[a-f0-9]{64}$/.test(row.reference_png_sha256) || !/^[a-f0-9]{64}$/.test(row.preview_png_sha256)) die("corpus row record is not immutable");
    row.svg = await bindLocalFile(assetsRoot, row.svg_path, row.svg_sha256, "source SVG"); row.input_png = await bindLocalFile(assetsRoot, row.input_png_path, row.png_sha256, "input PNG"); row.reference = await bindLocalFile(assetsRoot, row.reference_png, row.reference_png_sha256, "reference PNG"); row.preview = await bindLocalFile(assetsRoot, row.preview_png, row.preview_png_sha256, "preview PNG"); rows.push(row);
  }
  ...
  const route = (row, suffix, tier = "1b") => { const asset = imported.get(row.case_index); if (!asset?.asset_id || asset.input_png_sha256 !== row.input_png.sha256) die("imported project asset identity mismatches source row"); return { case_id: `quality-v1-${row.case_index}${suffix}`, projectId: binding.project_id, sourceAssetId: asset.asset_id, model: tier === "8b" ? "starvector_8b" : "starvector_1b", source_svg_sha256: row.svg.sha256, input_png_sha256: row.input_png.sha256, reference_png: row.reference.path, reference_png_sha256: row.reference.sha256, preview_png: row.preview.path, preview_png_sha256: row.preview.sha256, sampling: row.sampling, detailBudget: row.detail_budget }; };
  const tuples = Object.fromEntries(["mlx:1b", "mlx:8b", "candle-cuda:1b", "candle-cuda:8b"].map((tuple) => { const tier = tuple.split(":")[1]; return [tuple, { image_quality: rows.map((row) => route(row, "", tier)), deterministic_parity: rows.slice(0, 20).map((row, seed) => ({ ...route(row, "-parity", tier), seed })), lifecycle: index.lifecycle_cases?.[tuple], limits: index.limit_cases?.[tuple], run_identity: index.run_identity?.[tuple], hardware: index.hardware?.[tuple] }]; }));
  ...
  const bundle = { schema_version: 1, inference_revision: REV, corpus_sha256: sha(await readFile(corpusPath)), row_identity_sha256: rowHash, tuples, hostile_sanitizer, prompt_composition: index.prompt_composition, terminal_suite_identity: index.terminal_suite_identity };
