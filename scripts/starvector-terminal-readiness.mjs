...
export async function validateCorpusAssets(inferenceRoot, corpusRelative, assetsRoot, permanentPin) {
  ...
  for (const [position, row] of index.rows.entries()) {
    ...
    for (const [tier, expected] of [["1b", 7933], ["8b", 15422]]) {
      ...
      if (budget?.maxNewTokens !== expected || budget.maxSvgBytes !== 262144 || budget.maxWallTimeMs !== 300000) die(`terminal row ${position} lacks the ${tier} shipping Detail budget`);
    }
