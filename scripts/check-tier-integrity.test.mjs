...
test("the lane oracle reads the routing tables, not the manifest's per-backend block", async () => {
  ...
  // says nothing about the lane. Reading block presence as lane existence is how sc-16462 concluded
  // chroma had no candle lane and deleted a live f32 residency.
  //
  // This used to name anima_base, which then GREW a `candle` block (sc-20799 gave the Anima candle
  // lanes their request-scoped declarations) and took the premise with it. So derive the witnesses
  // structurally instead of hardcoding one model: every entry the router serves on candle while its
  // manifest carries no `candle` block at all. The population is what matters, not its membership —
  // a model gaining a tuning block just moves it out of the set instead of reddening the branch.
  const candleRoutedWithoutBlock = inputs.manifest.models
    .filter((entry) => entry.candle === undefined && lanes.get(entry.id)?.has("candle"))
    .map((entry) => entry.id);
  assert.ok(
    candleRoutedWithoutBlock.length > 0,
    "no model is candle-routed while lacking a manifest `candle` block, so this test can no longer " +
      "discriminate block-presence from lane-existence — re-anchor it rather than deleting it",
  );
  for (const id of candleRoutedWithoutBlock) {
    assert.equal(byId.get(id).candle, undefined, `fixture premise: ${id} has no candle block`);
    assert.ok(
      lanes.get(id)?.has("candle"),
      `${id} is candle-routed; a missing manifest block must not hide that`,
    );
  }
