...
import { chmod, copyFile, link, lstat, mkdir, mkdtemp, readFile, rename, rm, stat, symlink, writeFile } from "node:fs/promises";
...
test("readiness binds all 120 source assets and every suite identity to the pinned corpus", async () => {
  ...
  const acquire = (url, destination, expected) => downloadExact(url, destination, expected, async () => { requests++; return new Response("parquet"); });
  assert.equal(await prepareCorpusInputs(options, acquire), "reuse");
  assert.equal(requests, 0, "valid120-row corpus must need no network requests");
  assert.equal(await lstat(parquetRoot).catch(() => null), null, "reuse must not create source input directories");
  await rename(assets, `${assets}-saved`);
  assert.equal(await prepareCorpusInputs(options, acquire), "materialize");
  assert.equal(requests, 4, "missing materialization acquires exactly the four pinned sources");
  assert.equal(await prepareCorpusInputs(options, acquire), "materialize");
  assert.equal(requests, 4, "verified Parquet inputs are reused too");
  await writeFile(path.join(parquetRoot, "source-0.parquet"), "tampered parquet");
  await assert.rejects(() => prepareCorpusInputs(options, acquire), /existing download differs/);
  assert.equal(requests, 4, "drifted source input must not be silently overwritten");
  await rename(`${assets}-saved`, assets);
  await rm(path.join(assets, "input.png"));
  await assert.rejects(() => prepareCorpusInputs(options, acquire), /ENOENT/);
  assert.equal(requests, 4, "incomplete existing materialization is rejected without network");
  ...
  await assert.rejects(() => prepareCorpusInputs(options, acquire), /input PNG hash mismatch/);
