...
| C4 | Packaged Candle video curve | **Resolved for the exact measured selector:** the bundle contains one MLX curve and one Candle curve. The Candle curve comes only from artifact `9474424146` (run `32564176646`, job `97010255064`) at capture head `1ea0cc2d1365b7e7f691b9331346cd0a329c900e`, with six `runtime_complete` records, four fit plus two held out. It binds only the exact Wan q4 routed model/provider identity; foreign or unmatched Candle routes remain Unverified, and no synthetic evidence is retained. |
