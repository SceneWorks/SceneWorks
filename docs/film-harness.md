...
[film-harness-overview.md](film-harness-overview.md) is the orientation for someone who has never run
it: what problem the harness solves, the three documents, the run loop, the measured costs and the
non-goals. [film-editor.md](film-editor.md) is the current operator guide for the Editor's Film
workspace. [film-harness-ui-integration.md](film-harness-ui-integration.md) covers the shared
library/CLI/route layering and durable run record.

Epic 22708 / sc-22710, sc-22711, sc-22712, sc-22713, sc-22714, sc-22715. Renders a production plan — hand-authored, or
generated by the CLI's local planner from a brief — into a SceneWorks sequence through the
existing API seams: projects, asset import, `POST /api/v1/prompts/refine`,
`POST /api/v1/video/jobs`, job polling, timelines and the `timeline_export` job. It leaves versioned
documents behind at every step. The CLI and Film workspace share this orchestration; every take is
produced by the worker that claims its job. The editor also offers explicit external planning and
separate export without changing the CLI commands documented here.
...
The assembled timeline has `track_main` for picture and ordered audio lanes for dialogue,
ambience, music and each placed effect. Each audio track is a bus with its own `gain` and `muted`;
**the beds are placed once for the whole sequence**, so they play straight through the cuts instead
of restarting at each one. A later incremental delivery extends an untouched full-sequence bed to
the new picture duration while preserving gain, mute, fades and item volume. An explicit placement,
source-range or duration trim prevents that extension and remains exactly as the editor saved it.
The export mixes every non-muted audio track — gain is
...
### Film editor sound controls

The Film workspace exposes the film-required editor half on the same timeline contract: dialogue
text and voices or prerecorded lines, ambience, music and effects; placement, source trim, fades,
item volume, track gain and mute; audible preview; and persisted provenance. Completed shots and
sound arrive incrementally without overwriting an editor's saved placement, lane order or deleted
items. Rendering never exports implicitly. **Export current cut** is the explicit action that saves
and renders the current timeline, reports its status and failure, and lists any dropped audio layer.
This does not claim that every general-purpose audio-editing feature outside the Film workspace is
part of the harness.
...
- **The CLI remains local/private.** `film-harness` refuses before its first token if the environment
  carries a hosted LLM credential or endpoint (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`,
  `SCENEWORKS_LLM_BASE_URL`, …) or if `--api` is not loopback, a private-network address, an
  `.local` name or a bare hostname. Since sc-22715 that rule guards every CLI command that reaches
  an API from the one place the binary builds its transport (`guarded_transport`). The Film
  workspace separately supports a saved OpenAI-compatible planner connection as an explicit
  operator choice. It shows a data disclosure, keeps the secret in backend storage, sends reference
  pixels only after a separate opt-in, and never falls back to another provider.
...
- **Bounded repair.** Each round hands the validator's findings back verbatim and asks for the whole
  ...
  repair may change the number of shots or select other menu-valid durations to meet the brief's
  running-time window, but it must retain every required beat and pass the same full validation.
- **Workspace planner selection is separate from the video model.** New drafts use the built-in
  `prompt_refine_anubis_8b` planner without downloading Qwen. The optional local
  `film_planner_qwen3_6_27b` is available only after an explicit install and selection. Planner
  provider, actual LLM model identity and thinking mode are persisted independently of the target
  video model. An unavailable, canceled or exhausted operation preserves the draft and manual
  editing path; it does not render or substitute another planner. See
  [film-editor.md](film-editor.md) for the UI flow.
...
**One controller per run directory.** Every mutating CLI or hosted controller holds a
`ControllerLease` advisory lock. Running `run`, `resume`, review or an edit against an actively held
directory is refused before it can rewrite `run.json`. Clean release clears the lease's owner
marker; an abrupt process exit releases the OS lock and leaves the marker so API startup can adopt
only that interrupted controller. A cleanly released resumable run waits for an explicit Resume.
Idempotency keys still close the different window between posting a job and recording its id.
`replace-take` also refuses while the shot has an unsettled attempt and points at `resume`.
...
**Recorded ids survive queue clearing.** A job id already stored in `run.json` is reconciled through
`GET /api/v1/jobs/:id`, so hiding it from the queue listing does not cause a duplicate dispatch.
The idempotency-key lookup uses `GET /api/v1/jobs` only for the pre-id crash window; that listing
filters cleared jobs, is capped at 500 and has no offset, so a full page is widened by status and
merged by job id. This is another reason the record, rather than the queue list, is the run's state.
...
are not re-spent: a cancel is not a retry. The controller waits at most 30 seconds, capped by the
shot's `maxShotSeconds`, for the worker to honor cancellation; if it does not, the run stops without
dispatching another render beside the active job. A controller killed any other way (SIGKILL, a crash,
