...
**An adapter-source edit does NOT require a regeneration here.** Worth stating because it was true
until recently and the instinct is wrong in a useful direction. The deleted cost model
(`docs/generated/calibration-cost-model.json`, removed by sc-18100) fingerprinted the adapter sources
`crates/sceneworks-memory-adapter/src/bin/{mlx,candle}.rs`, so editing an adapter with no capture at
all used to red its `--check`; that was measured in sc-18104, where a comment-and-`match` edit to
`mlx.rs` turned it red and regenerating produced a two-line diff. `docs/generated/memory-matrix.json`
did **not** move on that same edit and does not read adapter sources, so with the cost model gone
nothing in §8 is downstream of them. Regenerate here because you ingested evidence or moved bindings
— not because you touched Rust. (What an adapter edit *does* still owe: `cargo fmt --all`, and a
clippy-clean build, since the macOS lane lints that binary.)
