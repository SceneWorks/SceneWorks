...
**Preview-support states** (sc-16965, epic 16948): "no frame yet" and "this route cannot preview" used to render identically, so a user waiting out model load + text encode — tens of seconds on a cold cache — had no way to know whether to keep waiting. The card now reads `preview.byBackend` off the model's catalog entry, keyed by the backend of the worker that claimed the job (`previewSupport.js`), and publishes the result on the article as `data-preview-state`:

| `data-preview-state` | Meaning | Thumbnails |
| --- | --- | --- |
| `live` | a streamed frame is on a running job | the frame, in the interim cell |
| `pending` | the route previews and no frame can have arrived yet — pre-denoise, or the first denoise step | a "Preview starting…" placeholder in the same interim slot the frame will take |
| `supported` | the route previews, but we are past that window or the job is terminal | unchanged |
| `unsupported` | the route is wired and does **not** preview | unchanged — no placeholder, no explanatory chrome |
| `unknown` | the catalog has no answer for this model/backend | unchanged |

Engine-**keyed**, never one boolean: the same model id answers differently per backend during the epic 16948 rollout, and it never fully collapses (SenseNova-U1 is candle-only and MLX never wired it). The flag is served from `config/manifests/builtin.preview-support.jsonc`, generated by `npm run gen:preview-support` from the checked-in `config/engine-capabilities/capabilities.<backend>.json` dumps joined through `engines.rs`'s `MODEL_TABLE`; `previewSupportCatalog.test.js` guards it on every PR. The separate **audio** registry is dumped alongside into `config/engine-capabilities/audio/` (sc-17593) and joined by the identity — an audio engine id *is* the SceneWorks model id — so the catalog answers `false` rather than `unknown` for audio routes; the card itself is unaffected, because `PREVIEW_JOB_TYPES` gates the placeholder to image jobs. The placeholder is bounded by the pre-denoise stage set **plus the first denoise step**, so a route that advertises support but emits nothing degrades to the pre-sc-16965 appearance instead of holding a placeholder for the whole render. The extra step is not slack: `GenEvent::Preview` deliberately does not POST — the frame rides the *next* `Step`/`Decoding` update (`image_jobs/base.rs`) and the sampler emits `Progress::Step` before the preview hook — so the first `generating` POST is always frame-less, and bounding on the stage set alone blinked the cell away for exactly one step before the frame restored it.
