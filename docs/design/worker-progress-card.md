...
| `pending` | the route previews, and denoise has not reported yet | a "Preview starting…" placeholder in the same interim slot the frame will take |
| `supported` | the route previews, but we are past the pre-denoise window or the job is terminal | unchanged |
