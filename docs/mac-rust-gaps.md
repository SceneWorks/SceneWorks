...
| Image upscaler (standalone) | `image_upscale` | Real-ESRGAN / AuraSR (torch) | ✅ Ported (Real-ESRGAN via Rust `ort`/CoreML, macOS MLX worker). **AuraSR engine = DROPPED on Mac** (sc-3668 port-or-drop spike: 617M torch-only GigaGAN, no viable Rust path, only a marginal & ~35–50× slower quality difference vs Real-ESRGAN x4) — UI-gated out of the Mac engine picker; stays available on Windows/Linux | sc-3489 (Real-ESRGAN); **sc-3668 (AuraSR drop)** |
