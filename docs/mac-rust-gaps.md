...
| Image upscaler (standalone) | `image_upscale` | Real-ESRGAN / AuraSR (torch) | ✅ Ported. **Real-ESRGAN** via Rust `ort`/CoreML or CUDA; **SeedVR2** (`engine=seedvr2`) = one-step diffusion super-resolution through native MLX on Mac (`mlx-gen-seedvr2`, sc-4815) and native Candle/CUDA on Windows/Linux (`candle-gen-seedvr2`, sc-5928/sc-5160). Both share the factor/detail contract. **AuraSR = DROPPED as an offered engine** (sc-3668/sc-5499). SeedVR2 HD spatial tiling remains a tracked follow-up | sc-3489 (Real-ESRGAN); **sc-4815 + sc-5928/sc-5160 (SeedVR2, epic 4811/5482)**; **sc-3668 (AuraSR drop)** |
| Video upscaler (standalone) | `video_upscale` | **none (net-new capability — SceneWorks had no video upscaler)** | ✅ **SeedVR2** on both native backends: MLX on Mac (epic 4811 / sc-4816) and Candle/CUDA on Windows/Linux (sc-5928 / sc-5160). Both decode the source clip → one-step super-resolution with temporal chunks/overlap → re-encode + source-audio passthrough. 7B/int8/HD-tiling remain sc-5197/5198/5201 | epic 4811 — engine sc-4813/sc-4814; worker cutover **sc-4816**; Candle **sc-5928/sc-5160** |
