...
4. **The same declared tier can yield different residency on different backends, and that is a
   ...
   **f32** under `candle-gen-sensenova`, which widens every dense leaf it multiplies against an f32
   ...
   hides an above-tier residency. The lane is validated, cross-checked against the lanes the **router**
   actually serves the model on, and published as a column of the generated audit; a per-backend claim
   that the audit does not show is a claim nobody can review.

   Naming the lane is **mandatory** (SC-17395). The field was optional until then, with omission meaning
   "both lanes" — and the lane cross-check ran only when the key was present, so the defaulted claim was
   the one nothing validated. Because every entry in the ledger is routed on both lanes, each omitting
   row was asserting that its `residentTier` and cost hold identically under two independently-written
   loaders, and nobody had read the second one. That is how the chroma rows carried candle's f32 T5
   figure on the mlx lane (2x, SC-16462) and how a `flux2_klein_9b_kv` candle row went missing while its
   sibling entry had one. A defaulted claim is unfalsifiable; a signed one is reviewable.

   The oracle for "does this lane exist" is the routing tables (`ModelCaps` / `VideoModelCaps` and the
   bespoke shape routes), **not** the manifest's per-backend block. That block is optional tuning config
   — only half the catalog carries a `candle` one, while entries with no block at all are generated by a
   registered candle provider — so reading its absence as "no lane" deletes live residency facts.
