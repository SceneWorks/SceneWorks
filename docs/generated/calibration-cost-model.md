...
- Matrix revision: `source-tree:4a6caa4d5492bcc4442b3325845fde1285c2011e115eb8d8cef520aa655b22d5`
- Inference revision: `65833f24409bce33c418499c2029bee76d842740`
...
| `no-provider-adapter` — No provider adapter covers this (entry, backend) at all | 2610 | 1640 | 970 | provider-adapter code |
| `adapter-gated` — An adapter exists, but it has never emitted a `complete` record | 90 | 30 | 60 | provider-adapter code (phase telemetry and lifecycle injection) |
...
| --- | --- | --- | --- |
| `no-provider-adapter` + `overlay-declined` | 4300 | 2710 | 1590 |
| `no-provider-adapter` | 2610 | 1640 | 970 |
| `adapter-gated` + `overlay-declined` | 174 | 60 | 114 |
| (none — producible today) | 90 | 90 | 0 |
| `adapter-gated` | 90 | 30 | 60 |
...
| --- | --- | --- | --- |
| `no-provider-adapter` | 6910 | 2610 | 4300 |
| `adapter-gated` | 264 | 90 | 174 |
| `overlay-declined` | 4564 | 90 | 4474 |

**`overlay-declined` is charged 4564 cells by the first-match partition, but it is the SOLE blocker on 90 of them. 4300 of them (94.2%) are also blocked by having NO PROVIDER ADAPTER AT ALL, and the remaining 174 by an adapter that has never emitted a `complete` record. Charge adapter coverage first and the overlay bucket falls to 264 while `no-provider-adapter` rises to 6910. PROVIDER-ADAPTER COVERAGE, not overlay support, is the largest independent blocker: it is the only thing wrong with 2610 cells and it touches 6910. Closing the remaining overlay-support gaps is still necessary, but on its own it moves 264 cells, not 4564.**
...
| --- | --- | --- |
| `adapter-gated` | 90 | 90 |
| `no-provider-adapter` | 2610 | 6910 |
| `no-run-needed` | 6 | 6 |
| `overlay-declined` | 4564 | 264 |
...
Ranked by **soleBlockerCells — cells this gate is the ONLY thing wrong with**, not by first-match bucket size. An earlier version of this document ranked the overlay gap #1 because it is the largest bucket in the ordered partition; 4474 of those cells are also blocked by having no adapter at all, so that ranking pointed remediation at work that moves 264 cells rather than 4564.

### 1. how much provider-adapter work the CATALOG COVERAGE gap needs — gate `no-provider-adapter`, **2610** cells independently blocked (6910 touched)

This is the largest INDEPENDENT blocker in the matrix: it is the only thing wrong with 2610 cells and it touches 6910 — 4474 of them jointly with the overlay gap. Only 4 of 53 catalog entries have a provider adapter at all, so most of the matrix cannot be measured by any code that exists, for any overlay. Charge this gate before overlay and it accounts for 6910 cells.
...
### 4. what a `complete` record actually requires from an existing adapter — gate `adapter-gated`, **90** cells independently blocked (264 touched)

90 cells are blocked ONLY by this, and 264 in total. The Krea MLX control adapter has now emitted complete exact records, proving the pipeline can close; this bucket applies only to adapter-covered pairs that still have no complete record.
