...
- Matrix revision: `source-tree:8f1b88f0a37ddb558bf5a37d77bef7351a48ec524af26e63a9513098d975a18f`
...
**7024** of 7030 cells (**99.9%**) are blocked by provider-adapter code that does not exist, not by machine time. **0** could be certified by a run started now.
...
| `overlay-declined` — Non-`none` overlay: no adapter can emit a truthful overlay scenario | 4354 | 2890 | 1464 | provider-adapter code |
...
| --- | --- | --- | --- |
| `no-provider-adapter` + `overlay-declined` | 4240 | 2830 | 1410 |
...
| --- | --- | --- | --- |
| `no-provider-adapter` | 6850 | 2610 | 4240 |
| `adapter-gated` | 174 | 60 | 114 |
| `overlay-declined` | 4354 | 0 | 4354 |

**`overlay-declined` is charged 4354 cells by the first-match partition, but it is the SOLE blocker on 0 of them. 4240 of them (97.4%) are also blocked by having NO PROVIDER ADAPTER AT ALL, and the remaining 114 by an adapter that has never emitted a `complete` record. Charge adapter coverage first and the overlay bucket falls to 114 while `no-provider-adapter` rises to 6850. PROVIDER-ADAPTER COVERAGE, not overlay support, is the largest independent blocker: it is the only thing wrong with 2610 cells and it touches 6850. Scoping overlay support in both adapters is still necessary, but on its own it moves 114 cells, not 4354.**
...
| --- | --- | --- |
| `adapter-gated` | 60 | 60 |
| `no-provider-adapter` | 2610 | 6850 |
| `no-run-needed` | 6 | 6 |
| `overlay-declined` | 4354 | 114 |
...
| --- | --- |
| `Implemented/unverified` | 2305 |
| `Missing` | 4719 |
| `Structurally N/A` | 6 |

Total **7030** cells across 1406 (entry, backend, tier, mode, overlay) session keys, every one of which spans exactly 5 rungs.
...
Geometry is already collapsed: keyed per advertised resolution the matrix would hold **43170** cells, so the published count embeds a **6.141x** collapse.
...
The overlay axis is **4360** cells — 62% of the matrix — and it is not uniformly real. `lora` and `identity` are genuine dimensions; `control` is nominal at 465 cells on 12 (entry, backend) pair(s).
...
| --- | --- | --- | --- |
| Certifying records (1 per cell) | 7024 | 4620 | 2404 |
| Warm sessions (model loads, 5 rungs each) | 1406 | 924 | 482 |
| Warm sessions, overlays also amortised — **NOT AVAILABLE in the shipped load contract; a price tag, not a plan** | 534 | 346 | 188 |

**Does the overlay axis evaporate? No — on either axis.** On the record axis, each of the 4360 non-`none` cells needs its own record, because `overlay` is in the cell key and `validateComplete` requires the overlay scenario to actually pass.

On the model-load axis the answer is **unavailable in the shipped load contract**. Dropping overlay from the session key would take loads from 1406 to 534 — a further 2.63x — but that figure **prices a capability the shipped code does not offer**, and an earlier version of this document wrongly reported it as "mostly yes". Four findings, the last decisive:
...
Irreducible floor: the **1406** `resident` cells. Rung 0 is the unoptimised baseline every model has by definition, so it can never be Structurally N/A. No survey outcome can remove these cells from the campaign.

| Scenario | Kind | Records | MLX | Candle | Warm sessions | Exempted MLX/Candle | ...if proportional |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `current` | fact | 7024 | 4620 | 2404 | 1406 | 0/0 | — |
| `rung4-half` | projection | 6324 | 4125 | 2199 | 1406 | 495/205 | 462/238 ⚠ |
| `rung4-all` | projection | 5624 | 3696 | 1928 | 1406 | 924/476 | 924/476 |
| `rungs-2-3-4-half` | projection | 4918 | 3137 | 1781 | 1406 | 1483/623 | 1386/720 ⚠ |
| `everything-but-baseline` | bound | 1406 | 924 | 482 | 1406 | 3696/1922 | 3696/1922 |
...
| --- | --- | --- | --- | --- | --- |
| 30 | 58.5 | 38.5 | 20 | 11.7 | 4.5 |
| 60 | 117.1 | 77 | 40.1 | 23.4 | 8.9 |
| 120 | 234.1 | 154 | 80.1 | 46.9 | 17.8 |
| 300 | 585.3 | 385 | 200.3 | 117.2 | 44.5 |
| 600 | 1170.7 | 770 | 400.7 | 234.3 | 89 |
| 1200 | 2341.3 | 1540 | 801.3 | 468.7 | 178 |
...
| --- | --- | --- |
| `exhaustive-exact-geometry` | 8634 | 43170 |
| `exhaustive-per-cell` | 1406 | 7024 |
| `fit-then-validate` | 2883 | 14415 |
...
Ratios: exact-geometry / fit = **2.99x**; exact-geometry / per-cell = 6.14x; fit / per-cell = 2.05x.
...
`geometryPointsPerFit`, `slopeSharedAcrossTiers` and `validationSampleFraction` were the only parameters in this model that were fixed rather than swept, which contradicted its own stated policy. Swept here. The exact-geometry/fit ratio ranges **1.44x–4.54x** across the grid, so the headline `2.99x` is one point on a surface.

| geometry points | slope shared | validation sample | Fit loads | Validation loads | Total loads | exact/fit | fit/per-cell |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 2 | false | 0 | 2812 | 0 | 2812 | 3.07x | 2x |
| 2 | false | 0.05 | 2812 | 71 | 2883 | 2.99x | 2.05x |
| 2 | false | 0.25 | 2812 | 352 | 3164 | 2.73x | 2.25x |
| 2 | true | 0 | 1900 | 0 | 1900 | 4.54x | 1.35x |
| 2 | true | 0.05 | 1900 | 71 | 1971 | 4.38x | 1.4x |
| 2 | true | 0.25 | 1900 | 352 | 2252 | 3.83x | 1.6x |
| 3 | false | 0 | 4218 | 0 | 4218 | 2.05x | 3x |
| 3 | false | 0.05 | 4218 | 71 | 4289 | 2.01x | 3.05x |
| 3 | false | 0.25 | 4218 | 352 | 4570 | 1.89x | 3.25x |
| 3 | true | 0 | 2394 | 0 | 2394 | 3.61x | 1.7x |
| 3 | true | 0.05 | 2394 | 71 | 2465 | 3.5x | 1.75x |
| 3 | true | 0.25 | 2394 | 352 | 2746 | 3.14x | 1.95x |
| 4 | false | 0 | 5624 | 0 | 5624 | 1.54x | 4x |
| 4 | false | 0.05 | 5624 | 71 | 5695 | 1.52x | 4.05x |
| 4 | false | 0.25 | 5624 | 352 | 5976 | 1.44x | 4.25x |
| 4 | true | 0 | 2888 | 0 | 2888 | 2.99x | 2.05x |
| 4 | true | 0.05 | 2888 | 71 | 2959 | 2.92x | 2.1x |
| 4 | true | 0.25 | 2888 | 352 | 3240 | 2.66x | 2.3x |
...
Ranked by **soleBlockerCells — cells this gate is the ONLY thing wrong with**, not by first-match bucket size. An earlier version of this document ranked the overlay gap #1 because it is the largest bucket in the ordered partition; 4354 of those cells are also blocked by having no adapter at all, so that ranking pointed remediation at work that moves 114 cells rather than 4354.

### 1. how much provider-adapter work the CATALOG COVERAGE gap needs — gate `no-provider-adapter`, **2610** cells independently blocked (6850 touched)

This is the largest INDEPENDENT blocker in the matrix: it is the only thing wrong with 2610 cells and it touches 6850 — 4354 of them jointly with the overlay gap. Only 2 of 53 catalog entries have a provider adapter at all, so most of the matrix cannot be measured by any code that exists, for any overlay. Charge this gate before overlay and it accounts for 6850 cells.
...
### 4. how much provider-adapter work the OVERLAY axis needs — gate `overlay-declined`, **0** cells independently blocked (4354 touched)

DEMOTED, and this is the correction that matters most in this document. The first-match partition charges this gate 4354 cells (59.5% of the matrix), which previously made it the #1 uncertainty. But it is the SOLE blocker on 0 of them: 4354 are also blocked by having no adapter or no `complete` record at all. Scoping overlay support in both adapters therefore moves 114 cells, not 4354. It remains necessary work — 3,925 cells cannot be certified without it — but it is not where the schedule is decided.
