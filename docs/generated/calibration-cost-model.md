...
- Matrix revision: `source-tree:f05daa946580b3a605d93419d220f86206accceb3a8329f23dc3f47b2941efa8`
- Inference revision: `942fcb44a68e39ffbb15d7a716ae3e766636ebd7`
...
| `no-provider-adapter` — No provider adapter covers this (entry, backend) at all | 2550 | 1640 | 910 | provider-adapter code |
| `adapter-gated` — An adapter exists, but it has never emitted a `complete` record | 150 | 30 | 120 | provider-adapter code (phase telemetry and lifecycle injection) |
...
| --- | --- | --- | --- |
| `no-provider-adapter` + `overlay-declined` | 4180 | 2710 | 1470 |
| `no-provider-adapter` | 2550 | 1640 | 910 |
| `adapter-gated` + `overlay-declined` | 294 | 60 | 234 |
| `adapter-gated` | 150 | 30 | 120 |
...
| --- | --- | --- | --- |
| `no-provider-adapter` | 6730 | 2550 | 4180 |
| `adapter-gated` | 444 | 150 | 294 |
| `overlay-declined` | 4564 | 90 | 4474 |

**`overlay-declined` is charged 4564 cells by the first-match partition, but it is the SOLE blocker on 90 of them. 4180 of them (91.6%) are also blocked by having NO PROVIDER ADAPTER AT ALL, and the remaining 294 by an adapter that has never emitted a `complete` record. Charge adapter coverage first and the overlay bucket falls to 384 while `no-provider-adapter` rises to 6730. PROVIDER-ADAPTER COVERAGE, not overlay support, is the largest independent blocker: it is the only thing wrong with 2550 cells and it touches 6730. Closing the remaining overlay-support gaps is still necessary, but on its own it moves 384 cells, not 4564.**
...
| `no-provider-adapter` | 2550 | 6730 |
| `no-run-needed` | 6 | 6 |
| `overlay-declined` | 4564 | 384 |
...
Completed-evidence baseline: 24 complete record(s) exist in the evidence bundle; the matrix reports 0 current calibration run(s) and 0 aggregate Verified cell(s). Exact records remain narrower than aggregate matrix cells, so a current exact run does not by itself promote a whole cell.
...
Ranked by **soleBlockerCells — cells this gate is the ONLY thing wrong with**, not by first-match bucket size. An earlier version of this document ranked the overlay gap #1 because it is the largest bucket in the ordered partition; 4474 of those cells are also blocked by having no adapter at all, so that ranking pointed remediation at work that moves 384 cells rather than 4564.

### 1. how much provider-adapter work the CATALOG COVERAGE gap needs — gate `no-provider-adapter`, **2550** cells independently blocked (6730 touched)

This is the largest INDEPENDENT blocker in the matrix: it is the only thing wrong with 2550 cells and it touches 6730 — 4474 of them jointly with the overlay gap. Only 5 of 53 catalog entries have a provider adapter at all, so most of the matrix cannot be measured by any code that exists, for any overlay. Charge this gate before overlay and it accounts for 6730 cells.
...
### 3. what a `complete` record actually requires from an existing adapter — gate `adapter-gated`, **150** cells independently blocked (444 touched)
