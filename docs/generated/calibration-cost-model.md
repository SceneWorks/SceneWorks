...
- Matrix revision: `source-tree:d49ee0afd19bd7cc2d7bd05b7aa81f388ac6b8a683b400d8eaa778643319a3ba`
...
| `no-provider-adapter` — No provider adapter covers this (entry, backend) at all | 2700 | 1700 | 1000 | provider-adapter code |
...
| --- | --- | --- | --- |
| `no-provider-adapter` + `overlay-declined` | 4480 | 2830 | 1650 |
| `no-provider-adapter` | 2700 | 1700 | 1000 |
...
| --- | --- | --- | --- |
| `no-provider-adapter` | 7180 | 2700 | 4480 |
| `adapter-gated` | 174 | 60 | 114 |
| `overlay-declined` | 4594 | 0 | 4594 |

**`overlay-declined` is charged 4594 cells by the first-match partition, but it is the SOLE blocker on 0 of them. 4480 of them (97.5%) are also blocked by having NO PROVIDER ADAPTER AT ALL, and the remaining 114 by an adapter that has never emitted a `complete` record. Charge adapter coverage first and the overlay bucket falls to 114 while `no-provider-adapter` rises to 7180. PROVIDER-ADAPTER COVERAGE, not overlay support, is the largest independent blocker: it is the only thing wrong with 2700 cells and it touches 7180. Scoping overlay support in both adapters is still necessary, but on its own it moves 114 cells, not 4594.**
...
| --- | --- | --- |
| `adapter-gated` | 60 | 60 |
| `no-provider-adapter` | 2700 | 7180 |
| `no-run-needed` | 6 | 6 |
| `overlay-declined` | 4594 | 114 |
...
Geometry is already collapsed: keyed per advertised resolution the matrix would hold **44490** cells, so the published count embeds a **6.045x** collapse.
...
| --- | --- | --- |
| `exhaustive-exact-geometry` | 8898 | 44490 |
...
Ratios: exact-geometry / fit = **2.95x**; exact-geometry / per-cell = 6.04x; fit / per-cell = 2.05x.
...
`geometryPointsPerFit`, `slopeSharedAcrossTiers` and `validationSampleFraction` were the only parameters in this model that were fixed rather than swept, which contradicted its own stated policy. Swept here. The exact-geometry/fit ratio ranges **1.42x–4.48x** across the grid, so the headline `2.95x` is one point on a surface.

| geometry points | slope shared | validation sample | Fit loads | Validation loads | Total loads | exact/fit | fit/per-cell |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 2 | false | 0 | 2944 | 0 | 2944 | 3.02x | 2x |
| 2 | false | 0.05 | 2944 | 74 | 3018 | 2.95x | 2.05x |
| 2 | false | 0.25 | 2944 | 368 | 3312 | 2.69x | 2.25x |
| 2 | true | 0 | 1988 | 0 | 1988 | 4.48x | 1.35x |
| 2 | true | 0.05 | 1988 | 74 | 2062 | 4.32x | 1.4x |
| 2 | true | 0.25 | 1988 | 368 | 2356 | 3.78x | 1.6x |
| 3 | false | 0 | 4416 | 0 | 4416 | 2.01x | 3x |
| 3 | false | 0.05 | 4416 | 74 | 4490 | 1.98x | 3.05x |
| 3 | false | 0.25 | 4416 | 368 | 4784 | 1.86x | 3.25x |
| 3 | true | 0 | 2504 | 0 | 2504 | 3.55x | 1.7x |
| 3 | true | 0.05 | 2504 | 74 | 2578 | 3.45x | 1.75x |
| 3 | true | 0.25 | 2504 | 368 | 2872 | 3.1x | 1.95x |
| 4 | false | 0 | 5888 | 0 | 5888 | 1.51x | 4x |
| 4 | false | 0.05 | 5888 | 74 | 5962 | 1.49x | 4.05x |
| 4 | false | 0.25 | 5888 | 368 | 6256 | 1.42x | 4.25x |
| 4 | true | 0 | 3020 | 0 | 3020 | 2.95x | 2.05x |
| 4 | true | 0.05 | 3020 | 74 | 3094 | 2.88x | 2.1x |
| 4 | true | 0.25 | 3020 | 368 | 3388 | 2.63x | 2.3x |
...
Ranked by **soleBlockerCells — cells this gate is the ONLY thing wrong with**, not by first-match bucket size. An earlier version of this document ranked the overlay gap #1 because it is the largest bucket in the ordered partition; 4594 of those cells are also blocked by having no adapter at all, so that ranking pointed remediation at work that moves 114 cells rather than 4594.

### 1. how much provider-adapter work the CATALOG COVERAGE gap needs — gate `no-provider-adapter`, **2700** cells independently blocked (7180 touched)

This is the largest INDEPENDENT blocker in the matrix: it is the only thing wrong with 2700 cells and it touches 7180 — 4594 of them jointly with the overlay gap. Only 2 of 53 catalog entries have a provider adapter at all, so most of the matrix cannot be measured by any code that exists, for any overlay. Charge this gate before overlay and it accounts for 7180 cells.
...
DEMOTED, and this is the correction that matters most in this document. The first-match partition charges this gate 4594 cells (59.5% of the matrix), which previously made it the #1 uncertainty. But it is the SOLE blocker on 0 of them: 4594 are also blocked by having no adapter or no `complete` record at all. Scoping overlay support in both adapters therefore moves 114 cells, not 4594. It remains necessary work — 3,925 cells cannot be certified without it — but it is not where the schedule is decided.
