...
- Matrix revision: `source-tree:d16e7576d7522e008330be0b28f543b74d889297c06e4480e50f9e64adb05308`
...
**230 of 7360 cells could be certified by a run started right now.**

**7124** of 7360 cells (**96.8%**) are blocked by provider-adapter code that does not exist, not by machine time. **230** could be certified by a run started now.
...
| `no-provider-adapter` — No provider adapter covers this (entry, backend) at all | 2410 | 1640 | 770 | provider-adapter code |
...
| --- | --- | --- | --- |
| `no-provider-adapter` + `overlay-declined` | 3890 | 2710 | 1180 |
| `no-provider-adapter` | 2410 | 1640 | 770 |
...
| --- | --- | --- | --- |
| `no-provider-adapter` | 6300 | 2410 | 3890 |
| `overlay-declined` | 4564 | 380 | 4184 |
| `adapter-gated` | 444 | 150 | 294 |

**`overlay-declined` is charged 4564 cells by the first-match partition, but it is the SOLE blocker on 380 of them. 3890 of them (85.2%) are also blocked by having NO PROVIDER ADAPTER AT ALL, and the remaining 294 by an adapter that has never emitted a `complete` record. Charge adapter coverage first and the overlay bucket falls to 674 while `no-provider-adapter` rises to 6300. PROVIDER-ADAPTER COVERAGE, not overlay support, is the largest independent blocker: it is the only thing wrong with 2410 cells and it touches 6300. Closing the remaining overlay-support gaps is still necessary, but on its own it moves 674 cells, not 4564.**
...
| `no-provider-adapter` | 2410 | 6300 |
| `no-run-needed` | 6 | 6 |
| `overlay-declined` | 4564 | 674 |
...
Ranked by **soleBlockerCells — cells this gate is the ONLY thing wrong with**, not by first-match bucket size. An earlier version of this document ranked the overlay gap #1 because it is the largest bucket in the ordered partition; 4184 of those cells are also blocked by having no adapter at all, so that ranking pointed remediation at work that moves 674 cells rather than 4564.

### 1. how much provider-adapter work the CATALOG COVERAGE gap needs — gate `no-provider-adapter`, **2410** cells independently blocked (6300 touched)

This is the largest INDEPENDENT blocker in the matrix: it is the only thing wrong with 2410 cells and it touches 6300 — 4184 of them jointly with the overlay gap. Only 9 of 53 catalog entries have a provider adapter at all, so most of the matrix cannot be measured by any code that exists, for any overlay. Charge this gate before overlay and it accounts for 6300 cells.
