...
- Matrix revision: `source-tree:f8ae79e7362fb50f700cc2a3b6daefccb56aeb47fbe5d0d1d77bc34da1f7ccb4`
- Inference revision: `bf06bb569697391a620e171f983fbb4d11a2ff14`
...
**120 of 7360 cells could be certified by a run started right now.**

**7234** of 7360 cells (**98.3%**) are blocked by provider-adapter code that does not exist, not by machine time. **120** could be certified by a run started now.
...
| `no-provider-adapter` — No provider adapter covers this (entry, backend) at all | 2520 | 1640 | 880 | provider-adapter code |
| `adapter-gated` — An adapter exists, but it has never emitted a `complete` record | 150 | 0 | 150 | provider-adapter code (phase telemetry and lifecycle injection) |
| `producible-today` — A run executed right now could certify this cell | 120 | 120 | 0 | nothing — GPU hours only |
...
| --- | --- | --- | --- |
| `no-provider-adapter` + `overlay-declined` | 4120 | 2710 | 1410 |
| `no-provider-adapter` | 2520 | 1640 | 880 |
| `adapter-gated` + `overlay-declined` | 294 | 0 | 294 |
| `adapter-gated` | 150 | 0 | 150 |
| `overlay-declined` | 150 | 150 | 0 |
| (none — producible today) | 120 | 120 | 0 |
| (exempt — no run needed) | 6 | 0 | 6 |

| Blocker | Cells touched | Sole blocker (fixing it alone frees these) | Co-blocked |
| --- | --- | --- | --- |
| `no-provider-adapter` | 6640 | 2520 | 4120 |
| `adapter-gated` | 444 | 150 | 294 |
| `overlay-declined` | 4564 | 150 | 4414 |

**`overlay-declined` is charged 4564 cells by the first-match partition, but it is the SOLE blocker on 150 of them. 4120 of them (90.3%) are also blocked by having NO PROVIDER ADAPTER AT ALL, and the remaining 294 by an adapter that has never emitted a `complete` record. Charge adapter coverage first and the overlay bucket falls to 444 while `no-provider-adapter` rises to 6640. PROVIDER-ADAPTER COVERAGE, not overlay support, is the largest independent blocker: it is the only thing wrong with 2520 cells and it touches 6640. Closing the remaining overlay-support gaps is still necessary, but on its own it moves 444 cells, not 4564.**
...
| `no-provider-adapter` | 2520 | 6640 |
| `no-run-needed` | 6 | 6 |
| `overlay-declined` | 4564 | 444 |
| `producible-today` | 120 | 120 |
...
Completed-evidence baseline: 33 complete record(s) exist in the evidence bundle; the matrix reports 0 current calibration run(s) and 0 aggregate Verified cell(s). Exact records remain narrower than aggregate matrix cells, so a current exact run does not by itself promote a whole cell.
...
Ranked by **soleBlockerCells — cells this gate is the ONLY thing wrong with**, not by first-match bucket size. An earlier version of this document ranked the overlay gap #1 because it is the largest bucket in the ordered partition; 4414 of those cells are also blocked by having no adapter at all, so that ranking pointed remediation at work that moves 444 cells rather than 4564.

### 1. how much provider-adapter work the CATALOG COVERAGE gap needs — gate `no-provider-adapter`, **2520** cells independently blocked (6640 touched)

This is the largest INDEPENDENT blocker in the matrix: it is the only thing wrong with 2520 cells and it touches 6640 — 4414 of them jointly with the overlay gap. Only 6 of 53 catalog entries have a provider adapter at all, so most of the matrix cannot be measured by any code that exists, for any overlay. Charge this gate before overlay and it accounts for 6640 cells.
...
It is a pure multiplier on every reported hour and it spans a 40x range in the sweep. The current artifact has 33 complete record(s) and 120 producible cell(s), but the records do not carry per-scenario duration telemetry, so they do not narrow that multiplier yet.
...
### 4. what a `complete` record actually requires from an existing adapter — gate `adapter-gated`, **150** cells independently blocked (444 touched)

150 cells are blocked ONLY by this, and 444 in total. The Krea MLX control adapter has now emitted complete exact records, proving the pipeline can close; this bucket applies only to adapter-covered pairs that still have no complete record.
