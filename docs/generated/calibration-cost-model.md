...
- Matrix revision: `source-tree:346b573b7a77102e4a53f0baf82bdb479ee5607fe16edeb1693d86a704fbfea0`
- Inference revision: `5b6d6aa02f9d85503d26e66a0732fcb1b841fa5b`
...
**170 of 7360 cells could be certified by a run started right now.**

**7184** of 7360 cells (**97.6%**) are blocked by provider-adapter code that does not exist, not by machine time. **170** could be certified by a run started now.
...
| `no-provider-adapter` — No provider adapter covers this (entry, backend) at all | 2470 | 1640 | 830 | provider-adapter code |
...
| `producible-today` — A run executed right now could certify this cell | 170 | 120 | 50 | nothing — GPU hours only |
...
| --- | --- | --- | --- |
| `no-provider-adapter` + `overlay-declined` | 4010 | 2710 | 1300 |
| `no-provider-adapter` | 2470 | 1640 | 830 |
| `adapter-gated` + `overlay-declined` | 294 | 0 | 294 |
| `overlay-declined` | 260 | 150 | 110 |
| (none — producible today) | 170 | 120 | 50 |
...
| --- | --- | --- | --- |
| `no-provider-adapter` | 6480 | 2470 | 4010 |
| `overlay-declined` | 4564 | 260 | 4304 |
| `adapter-gated` | 444 | 150 | 294 |

**`overlay-declined` is charged 4564 cells by the first-match partition, but it is the SOLE blocker on 260 of them. 4010 of them (87.9%) are also blocked by having NO PROVIDER ADAPTER AT ALL, and the remaining 294 by an adapter that has never emitted a `complete` record. Charge adapter coverage first and the overlay bucket falls to 554 while `no-provider-adapter` rises to 6480. PROVIDER-ADAPTER COVERAGE, not overlay support, is the largest independent blocker: it is the only thing wrong with 2470 cells and it touches 6480. Closing the remaining overlay-support gaps is still necessary, but on its own it moves 554 cells, not 4564.**
...
| `no-provider-adapter` | 2470 | 6480 |
| `no-run-needed` | 6 | 6 |
| `overlay-declined` | 4564 | 554 |
| `producible-today` | 170 | 170 |
...
Completed-evidence baseline: 33 Full complete and 10 base-only runtime-complete record(s) exist in the evidence bundle; the matrix reports 10 current calibration run(s) and 0 aggregate Verified cell(s). Exact records remain narrower than aggregate matrix cells, so a current exact run does not by itself promote a whole cell.
...
Ranked by **soleBlockerCells — cells this gate is the ONLY thing wrong with**, not by first-match bucket size. An earlier version of this document ranked the overlay gap #1 because it is the largest bucket in the ordered partition; 4304 of those cells are also blocked by having no adapter at all, so that ranking pointed remediation at work that moves 554 cells rather than 4564.

### 1. how much provider-adapter work the CATALOG COVERAGE gap needs — gate `no-provider-adapter`, **2470** cells independently blocked (6480 touched)

This is the largest INDEPENDENT blocker in the matrix: it is the only thing wrong with 2470 cells and it touches 6480 — 4304 of them jointly with the overlay gap. Only 8 of 53 catalog entries have a provider adapter at all, so most of the matrix cannot be measured by any code that exists, for any overlay. Charge this gate before overlay and it accounts for 6480 cells.
...
It is a pure multiplier on every reported hour and it spans a 40x range in the sweep. The current artifact has 43 activation-eligible record(s) and 170 producible cell(s), but the records do not carry per-scenario duration telemetry, so they do not narrow that multiplier yet.
