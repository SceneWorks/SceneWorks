...
- Matrix revision: `source-tree:cf00e38cb001521fda298ab68eea9330447cbbf4b268374e2f560927316ea2ef`
- Inference revision: `378f6c7dadd559bba7d32e8c87165e0dd7900710`
...
**ZERO cells are producible today. The binding constraint on this epic is provider-adapter code, not GPU hours: 0 provider pair(s) have emitted a `complete` record, while 7 provider pair(s) have an adapter at all. Every hour in the wall-clock sweep below is hypothetical until the remaining code gates change.**
...
| `overlay-declined` — Non-`none` overlay without a complete truthful overlay record | 4594 | 2890 | 1704 | provider-adapter code |
| `no-provider-adapter` — No provider adapter covers this (entry, backend) at all | 2550 | 1640 | 910 | provider-adapter code |
| `adapter-gated` — An adapter exists, but it has never emitted a `complete` record | 210 | 90 | 120 | provider-adapter code (phase telemetry and lifecycle injection) |
...
| `adapter-gated` + `overlay-declined` | 414 | 180 | 234 |
| `adapter-gated` | 210 | 90 | 120 |
...
| `adapter-gated` | 624 | 210 | 414 |
| `overlay-declined` | 4594 | 0 | 4594 |

**`overlay-declined` is charged 4594 cells by the first-match partition, but it is the SOLE blocker on 0 of them. 4180 of them (91%) are also blocked by having NO PROVIDER ADAPTER AT ALL, and the remaining 414 by an adapter that has never emitted a `complete` record. Charge adapter coverage first and the overlay bucket falls to 414 while `no-provider-adapter` rises to 6730. PROVIDER-ADAPTER COVERAGE, not overlay support, is the largest independent blocker: it is the only thing wrong with 2550 cells and it touches 6730. Closing the remaining overlay-support gaps is still necessary, but on its own it moves 414 cells, not 4594.**
...
| --- | --- | --- |
| `adapter-gated` | 210 | 210 |
| `no-provider-adapter` | 2550 | 6730 |
| `no-run-needed` | 6 | 6 |
| `overlay-declined` | 4594 | 414 |
...
Completed-evidence baseline: 0 complete record(s) exist in the evidence bundle; the matrix reports 0 current calibration run(s) and 0 aggregate Verified cell(s). Exact records remain narrower than aggregate matrix cells, so a current exact run does not by itself promote a whole cell.
...
It is a pure multiplier on every reported hour and it spans a 40x range in the sweep. The current artifact has 0 complete record(s) and 0 producible cell(s), but the records do not carry per-scenario duration telemetry, so they do not narrow that multiplier yet.
...
### 3. what a `complete` record actually requires from an existing adapter — gate `adapter-gated`, **210** cells independently blocked (624 touched)

210 cells are blocked ONLY by this, and 624 in total. The Krea MLX control adapter has now emitted complete exact records, proving the pipeline can close; this bucket applies only to adapter-covered pairs that still have no complete record.
