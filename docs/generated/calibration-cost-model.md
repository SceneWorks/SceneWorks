...
- Matrix revision: `source-tree:13933866a283f080861a563dfc5d45fa3a26a9f5ede81c554c2e2d0e51952ef4`
...
| `adapter-gated` — An adapter exists, but it has never emitted a `complete` record | 180 | 30 | 150 | provider-adapter code (phase telemetry and lifecycle injection) |
...
| `adapter-gated` + `overlay-declined` | 354 | 60 | 294 |
| `adapter-gated` | 180 | 30 | 150 |
...
| `adapter-gated` | 534 | 180 | 354 |
| `overlay-declined` | 4564 | 90 | 4474 |

**`overlay-declined` is charged 4564 cells by the first-match partition, but it is the SOLE blocker on 90 of them. 4120 of them (90.3%) are also blocked by having NO PROVIDER ADAPTER AT ALL, and the remaining 354 by an adapter that has never emitted a `complete` record. Charge adapter coverage first and the overlay bucket falls to 444 while `no-provider-adapter` rises to 6640. PROVIDER-ADAPTER COVERAGE, not overlay support, is the largest independent blocker: it is the only thing wrong with 2520 cells and it touches 6640. Closing the remaining overlay-support gaps is still necessary, but on its own it moves 444 cells, not 4564.**
...
Completed-evidence baseline: 28 complete record(s) exist in the evidence bundle; the matrix reports 0 current calibration run(s) and 0 aggregate Verified cell(s). Exact records remain narrower than aggregate matrix cells, so a current exact run does not by itself promote a whole cell.
...
It is a pure multiplier on every reported hour and it spans a 40x range in the sweep. The current artifact has 28 complete record(s) and 90 producible cell(s), but the records do not carry per-scenario duration telemetry, so they do not narrow that multiplier yet.
...
### 3. what a `complete` record actually requires from an existing adapter — gate `adapter-gated`, **180** cells independently blocked (534 touched)

180 cells are blocked ONLY by this, and 534 in total. The Krea MLX control adapter has now emitted complete exact records, proving the pipeline can close; this bucket applies only to adapter-covered pairs that still have no complete record.
