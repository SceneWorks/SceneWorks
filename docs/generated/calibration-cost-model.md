...
**ZERO cells are producible today. The binding constraint on this epic is provider-adapter code, not GPU hours: 0 provider pair(s) have emitted a `complete` record, while 3 provider pair(s) have an adapter at all. Every hour in the wall-clock sweep below is hypothetical until the remaining code gates change.**
