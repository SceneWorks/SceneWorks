...
- Matrix revision: `source-tree:dc4436e64161e40c66e8e4b7d4f1c938d97ae2d1aaf69c68e509742497e32618`
- Inference revision: `7410dea8364f184dbbb07a37c77c1c00e6039a23`
...
**180 of 7360 cells could be certified by a run started right now.**

**7174** of 7360 cells (**97.5%**) are blocked by provider-adapter code that does not exist, not by machine time. **180** could be certified by a run started now.
...
| `overlay-declined` — Non-`none` overlay without a complete truthful overlay record | 4504 | 2860 | 1644 | provider-adapter code |
...
| `producible-today` — A run executed right now could certify this cell | 180 | 90 | 90 | nothing — GPU hours only |
...
| (none — producible today) | 180 | 90 | 90 |
...
| `overlay-declined` | 4504 | 90 | 4414 |
| `adapter-gated` | 174 | 60 | 114 |

**`overlay-declined` is charged 4504 cells by the first-match partition, but it is the SOLE blocker on 90 of them. 4300 of them (95.5%) are also blocked by having NO PROVIDER ADAPTER AT ALL, and the remaining 114 by an adapter that has never emitted a `complete` record. Charge adapter coverage first and the overlay bucket falls to 204 while `no-provider-adapter` rises to 6910. PROVIDER-ADAPTER COVERAGE, not overlay support, is the largest independent blocker: it is the only thing wrong with 2610 cells and it touches 6910. Closing the remaining overlay-support gaps is still necessary, but on its own it moves 204 cells, not 4504.**
...
| `no-run-needed` | 6 | 6 |
| `overlay-declined` | 4504 | 204 |
| `producible-today` | 180 | 180 |
...
Completed-evidence baseline: 114 complete record(s) exist in the evidence bundle; the matrix reports 90 current calibration run(s) and 90 aggregate Verified cell(s). Exact records remain narrower than aggregate matrix cells, so a current exact run does not by itself promote a whole cell.
...
It is a pure multiplier on every reported hour and it spans a 40x range in the sweep. The current artifact has 114 complete record(s) and 180 producible cell(s), but the records do not carry per-scenario duration telemetry, so they do not narrow that multiplier yet.
...
### 3. how much provider-adapter work the OVERLAY axis needs — gate `overlay-declined`, **90** cells independently blocked (4504 touched)

DEMOTED, and this is the correction that matters most in this document. The first-match partition charges this gate 4504 cells (61.2% of the 7360-cell matrix), which previously made it the #1 uncertainty. But it is the SOLE blocker on 90 of them: 4414 are also blocked by having no adapter or no `complete` record at all. Scoping overlay support where it remains absent therefore moves 204 cells, not 4504. It remains necessary work — those 4504 cells cannot be certified without it — but it is not where the schedule is decided.
