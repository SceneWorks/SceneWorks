...
indexed read rebuilds from disk before serving data and then clears the marker.
...
path stats, directory scans, sidecar and generation-set reads, poster stats,
dirty-marker reads/writes/removes, directory-create calls, and DB opens.
`fs_total` is the sum of those categories. Counts are logical calls made by the
SceneWorks asset-list/reconciliation code; they do not claim to expose hidden
syscalls inside SQLite, the standard library, or the operating system.
...
The harness labels the first measurement `first-call` because it includes
`ProjectStore` registry-cache population, and later measurements
`steady-state-average`. It does not evict the OS filesystem cache and therefore
does not call the first measurement "cold storage." For a real cold-cache
comparison, evict or remount using the storage provider's supported procedure
before starting the process, and record that procedure with the output. The
generation-set, poster, registry, path, marker, directory, and DB counts remain
...
first-call: assets=500 elapsed_ms=5.571 fs_total=12 registry_opens=1 registry_metadata_reads=1 registry_content_reads=1 path_stats=1 directory_scans=0 sidecar_reads=0 generation_set_reads=1 poster_stats=0 index_marker_reads=1 index_marker_writes=0 index_marker_removes=0 directory_create_calls=3 db_opens=3
steady-state-average(10): assets=500 elapsed_ms=4.958 fs_total=12 registry_opens=1 registry_metadata_reads=1 registry_content_reads=1 path_stats=1 directory_scans=0 sidecar_reads=0 generation_set_reads=1 poster_stats=0 index_marker_reads=1 index_marker_writes=0 index_marker_removes=0 directory_create_calls=3 db_opens=3
```
