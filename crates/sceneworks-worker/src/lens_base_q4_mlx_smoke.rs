...
//! engine via `crate::inference_runtime::load("lens")` with a **Q4** `LoadSpec` pointed at the packed `q4/` turnkey
...
fn lens_base_q4_mlx_gpu_smoke() {
    ...
    // Same seam as the worker's MLX image path: a catalog load of the base `lens` generator + a Q4
    // `LoadSpec`
    ...
    let generator =
        crate::inference_runtime::load("lens", &spec).expect("load mlx lens (base) generator");
