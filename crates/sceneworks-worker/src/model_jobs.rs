...
pub(crate) async fn run_model_import_job(
        ...
        .cloned()
    {
        ...
        // `paths.model` is stamped only for an entry this job actually INSTALLED into
        // `target_dir`. A plan-backed entry (`importPlan.checkpointId`, epic 20398) is served from
        // its own plan layers and is claimed by exactly one lane on the absence of an install
        // path, so writing one here would both name a directory nothing installed and hand the
        // entry to a bespoke imported lane as well (sc-20635).
        if sceneworks_core::jobs_store::checkpoint_plan_checkpoint_id(&manifest_entry).is_none() {
