//! On-device build helper for the Wan2.2 I2V-A14B quant matrix (sc-9943, epic 8506).
//!
...
//! takes minutes per tier on an Apple-Silicon Mac. Run one-off to produce the artifacts, then
//! `hf upload` the `bf16/`/`q8/`/`q4/` subdirs to `SceneWorks/wan2.2-i2v-a14b-mlx`.
//!
//! ```sh
//! # Native checkpoint (high_noise_model/ + low_noise_model/ + models_t5_umt5-xxl-enc-bf16.pth +
//! # Wan2.1_VAE.pth), e.g. `hf download Wan-AI/Wan2.2-I2V-A14B --local-dir <native>`:
//! export SCENEWORKS_WAN_I2V_14B_NATIVE_DIR=<native>
//! export SCENEWORKS_WAN_I2V_14B_TIER_OUT=<out-root>          # bf16/ q8/ q4/ written here
...
//! # SceneWorks/wan2.2-i2v-a14b-mlx turnkey's tokenizer.json.
//! export SCENEWORKS_WAN_I2V_14B_TOKENIZER=<path/to/tokenizer.json>   # optional
//! cargo test -p sceneworks-worker --release wan_i2v_14b_build_tiers -- --ignored --nocapture
//! ```
//!
...
/// I2V checkpoint ships it), then the cached `SceneWorks/wan2.2-i2v-a14b-mlx` turnkey's
...
fn resolve_tokenizer(native_dir: &Path) -> PathBuf {
    if let Ok(explicit) = std::env::var("SCENEWORKS_WAN_I2V_14B_TOKENIZER") {
        let path = PathBuf::from(explicit.trim());
        assert!(
            path.is_file(),
            "SCENEWORKS_WAN_I2V_14B_TOKENIZER={} is not a file",
            path.display()
        );
        return path;
    }
    ...
    // Upstream I2V native layout ships the UMT5 tokenizer under google/umt5-xxl/.
    let native_umt5 = native_dir.join("google/umt5-xxl/tokenizer.json");
    if native_umt5.is_file() {
        return native_umt5;
    }
    // Cached turnkey: ~/.cache/huggingface/hub/models--SceneWorks--wan2.2-i2v-a14b-mlx/snapshots/*/tokenizer.json
    if let Some(home) = std::env::var_os("HOME") {
        let snapshots = PathBuf::from(home)
            .join(".cache/huggingface/hub")
            .join("models--SceneWorks--wan2.2-i2v-a14b-mlx")
            .join("snapshots");
    ...
    panic!(
        "no tokenizer.json found — set SCENEWORKS_WAN_I2V_14B_TOKENIZER, place one at {}, or cache \
         the SceneWorks/wan2.2-i2v-a14b-mlx turnkey (hf download … --include tokenizer.json)",
        beside.display()
    );
}

/// Build all three Wan2.2 I2V-A14B tier subdirs from the native checkpoint. `#[ignore]`d — run
/// on-device with the env vars above; not exercised in CI (needs the native weights).
#[test]
#[ignore = "on-device tier build: needs the ~126GB native Wan2.2-I2V-A14B checkpoint + minutes/tier"]
fn wan_i2v_14b_build_tiers() {
    let native_dir = PathBuf::from(
        std::env::var("SCENEWORKS_WAN_I2V_14B_NATIVE_DIR")
            .expect("set SCENEWORKS_WAN_I2V_14B_NATIVE_DIR to the native checkpoint dir"),
    );
    assert!(
    ...
    let out_root = PathBuf::from(
        std::env::var("SCENEWORKS_WAN_I2V_14B_TIER_OUT")
            .expect("set SCENEWORKS_WAN_I2V_14B_TIER_OUT to the output root for bf16/ q8/ q4/"),
    );
    let tokenizer = resolve_tokenizer(&native_dir);
    std::fs::create_dir_all(&out_root).unwrap();

    // The converter loads each expert as a full buffer then casts to bf16; at MLX's default cache
    // limit the freed buffers are RETAINED as cache, so after both experts the residue plus the T5
    // load OOM-SIGKILLs the process on a 128 GB box (sc-5567 batch pattern). Cap the buffer cache to
    // 0 so freed buffers return to the OS immediately between stages — a build-time realloc-cost
    // trade the generation hot path deliberately avoids.
