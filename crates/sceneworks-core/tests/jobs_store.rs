...
fn mac_rust_supported_names_torch_only_image_model_with_its_port_epic() {
    ...
    let cases = [
        ...
        // SenseNova-U1 (sensenova_u1_8b[_fast]) was ported to MLX (epic 3180 / sc-3900): its image
        // modes are now MLX-routed, so it is no longer a torch-only image model. Its VQA / interleave
        // understanding surface (a separate job type) is still gap-classified to epic 3180 below.
...
fn non_mlx_model_image_job_is_not_routed_to_mlx_worker() {
    ...
    // PuLID have no MLX crate) stays on the Python path: the torch worker
