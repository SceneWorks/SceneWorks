    ...
    let items = input
        ...
        .map(|item| {
            Ok(TrainingPlanItem {
                ...
                width: item.width,
                height: item.height,
                // Preserve model-specific per-example inputs. The worker forwards these losslessly
                // to the native trainer; LTX-2.5 uses `ltxPreparedBundlePath` for its preprocessed
                // video/audio latents and conditioning tensors. Paths are resolved and confined to
                // the dataset root by the worker before execution.
