...
mod tests {
    ...
    ///
    /// `sources` records which authoritative inputs were captured and their hashes when the checked-in
    /// artifact was produced. A non-capability edit inside one of those whole-file inputs legitimately
    /// changes its live hash without changing a capability cell. The key set and digest shape remain
    /// strict so sources cannot disappear, appear unreviewed, or carry malformed provenance; every
