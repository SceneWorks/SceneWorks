...
/// Where the FLUX.2-dev engine registers its ONLY memory-strategy contract at the pin. The
/// text-to-image provider exposes none (`contract_for_variant(Flux2Variant::Dev, ..) == None`), so
/// admission-scenario attestation for this lane goes through the crate's registered dev contract —
/// the same Resident-only, multi-reference-edit-gated contract the worker's
/// `flux2_dev_edit_memory_context` (image_jobs/flux2.rs) consults in production. The scenario
/// contexts below are therefore edit-shaped (mode Edit, two references), because that is the only
/// route the contract's own gate admits; the memory measurement itself is the loaded
/// `flux2_dev` text-to-image render.
const FLUX2_CONTRACT_PROVIDER: &str = "flux2_dev_edit";
...
/// One fixed seed for every `mlx:flux2_dev` fixture (`flux2-dev-mlx-<tier>-<edge>-seed18218-step2`).
...
mod tests {
    ...
    ///
    /// Two properties, each necessary (sc-18250 tightened this after review found the first draft
    /// iterated the SAME constants the match arms use, so corrupting a constant's VALUE passed while
    /// the live harness — which sends the plan's literal provider ids — was refused):
    ///
    ...
    ///
    /// This still cannot see a WRONG-ARM swap (two constants exchanging values would route each
    /// lane into the other's arm before its first field complaint); the per-arm provider guards
    /// (`validate_flux2_target`, `validate_z_image_batch`) are what refuse a misrouted target.
    #[test]
...
/// plan carries the same tier at two geometries (768² and 1024²) plus a reduced-geometry bf16 row.
...
/// calibrated load shape), and the quant DERIVED from the planned selection — `None` loads the
/// dense bf16 tree.
...
/// The admission context for the FLUX.2-dev safety scenarios. It is EDIT-SHAPED (mode Edit, two
/// references) because that is the only route the pinned contract's own gate admits — the same
/// shape the worker's production `flux2_dev_edit_memory_context` submits. `overlay` stays `None`:
/// gen-core rejects any overlay containing `=` as a structured-data smell, and the calibration
/// target declares no overlay.
...
///
/// At pin 40fa7583 `mlx-gen-flux2/src/memory_strategy.rs` marks every non-Resident strategy
/// `Missing` on its one contract, the text-to-image provider registers no contract at all, the
/// crate has no calibration fault-injection site, and `begin_memory_strategy_request` opens no
/// scope for the dev variants. Three consequences, each attested truthfully rather than imitated:
///
///   1. no per-rung machinery — a non-resident rung is refused by name below;
///   2. the scoped lifecycle scenarios (warm_repeat / cancel / error) cannot execute, so the
///      fragment is `runtime_complete` — the same status the lane's Candle twin records carry —
///      with those scenarios explicitly `not_run` and the negative mutation held as an
///      arm-internal falsifiability check (a runtime_complete record must keep the field null);
///   3. admission scenarios run through the crate's registered dev contract
///      (`FLUX2_CONTRACT_PROVIDER`) via the same weights-free function the loaded edit generator
///      delegates to (`registered_dev_safety_check`).
fn run_flux2_dev(request: &Value) -> Result<Value, String> {
    ...
    let selection = planned_selection(request)?;
    let tier = planned_qwen_tier(request)?; // the shared bf16|q4|q8 parser, not a Qwen-only fact
    let (width, height) = protocol::target_geometry(request)?;
    ...
    let contract = registry
        .memory_strategy_contract(FLUX2_CONTRACT_PROVIDER, &spec)
        .map_err(|error| {
            format!("read {FLUX2_CONTRACT_PROVIDER} memory-strategy contract: {error}")
        })?
        .ok_or_else(|| {
            format!("{FLUX2_CONTRACT_PROVIDER} has no memory-strategy contract at the pin")
        })?;
    ...
    let safety = |fingerprint: &str, total_bytes: u64, predicted: u64| {
        mlx_gen_flux2::memory_strategy::registered_dev_safety_check(
            &spec,
            &contract,
...
mod flux2_tests {
    ...
    /// sc-17097's lesson applied to this arm: the tier is derived from the planned target, and every
    /// declared tier must map to its own quant — bf16 loads dense (`None`), never a hardcoded q4.
    #[test]
    ...
    /// Pins the arm's three load-bearing premises to the PINNED provider crate, weights-free:
    ///
    ///   1. the text-to-image provider registers NO contract — the reason the arm reads its
    ///      admission surface from `FLUX2_CONTRACT_PROVIDER` at all;
    ///   2. that contract is resident-only (every other strategy `Missing`) — the reason the arm
    ...
    #[test]
    fn the_pinned_flux2_contract_is_resident_only_and_registered_on_the_edit_provider() {
        ...
        assert!(
            registry
                ...
                .unwrap()
                .is_none(),
            "the pinned flux2_dev text-to-image provider now registers a contract; the arm should \
             read it directly instead of {FLUX2_CONTRACT_PROVIDER}'s"
        );
        let contract = registry
            .memory_strategy_contract(FLUX2_CONTRACT_PROVIDER, &spec)
            .unwrap()
            .expect("the pinned FLUX.2-dev edit contract");
    ...
    /// registered function the loaded edit generator delegates to. Mutation-verified in both
    ...
    fn flux2_admission_scenarios_accept_exact_fit_and_reject_unknown_stale_and_foreign_routes() {
        ...
        let check = |context: &MemoryRunContext| {
            mlx_gen_flux2::memory_strategy::registered_dev_safety_check(&spec, &contract, context)
        };
        ...
        // The route-gate mutation: the same fitting budget in a text-to-image shape must be
        // refused — the contract admits only the multi-reference edit route.
        let mut text_to_image = flux2_admission_context(
            &selection,
            calibration,
            &calibration.fingerprint,
            1024,
        ...
        text_to_image.mode = MemoryMode::TextToImage;
        text_to_image.has_reference = false;
        text_to_image.geometry.reference_count = 0;
        assert!(matches!(
            check(&text_to_image),
            MemorySafetyDecision::Reject { .. }
        ));
