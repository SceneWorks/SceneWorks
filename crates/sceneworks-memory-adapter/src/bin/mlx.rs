    ...
    fn every_receipt_builder_is_bound_to_its_lane_prediction_wrapper() {
        ...
        assert_eq!(
            ...
            "all eight MLX image receipt builders must use the image policy wrapper \
             (sc-22726 added the FLUX.1 family arm; sc-22730 the shared SD3.5 arm)"
        );
        assert_eq!(
            source
                .lines()
...
struct Sd3Arm {
    ...
    root_env: &'static str,
    expected_repository: &'static str,
    /// The production calibration identity `mlx-gen-sd3` publishes for a clean base load of this
    /// route's shipped turnkey (`memory_strategy::production_calibration_fingerprint`).
    ///
    /// Checked-in here so a capture that loads a DIFFERENT identity — the weights-free registry
    /// string `sd3-5-mlx-registry-behavior-v2-*`, or the measured
    /// `sd3-5-large-bf16-mlx-shared-ladder-2026-08-03-v1` that belongs to the exact content-pinned
    /// SC-15522 snapshot rather than to the shipped rehost — is refused by name rather than emitted
    /// as this cell's evidence. The plan rows are bound to the same strings by the JS/Rust
    /// cross-source test in `scripts/measure-memory-catalog.test.mjs`.
...
const SD3_LARGE_ARM: Sd3Arm = Sd3Arm {
    ...
    calibration_fingerprint: "sd3-5-large-mlx-shared-ladder-v1",
    slug: "sd3-5-large",
    guidance: Some(3.5),
};
...
const SD3_LARGE_TURBO_ARM: Sd3Arm = Sd3Arm {
    ...
    calibration_fingerprint: "sd3-5-large-turbo-mlx-shared-ladder-v1",
...
const SD3_MEDIUM_ARM: Sd3Arm = Sd3Arm {
    ...
    calibration_fingerprint: "sd3-5-medium-mlx-shared-ladder-v1",
    slug: "sd3-5-medium",
    guidance: Some(5.0),
};
...
fn run_sd3(request: &Value) -> Result<Value, String> {
    ...
    if calibration.fingerprint != arm.calibration_fingerprint {
        return Err(format!(
            ...
            arm.provider, calibration.fingerprint, arm.calibration_fingerprint
        ));
    }
    let planned_fingerprint = protocol::planned(request)?
        .get("calibrationFingerprint")
...
mod sd3_tests {
    ...
    /// a provider whose published identity differs from `arm.calibration_fingerprint`.
    #[test]
