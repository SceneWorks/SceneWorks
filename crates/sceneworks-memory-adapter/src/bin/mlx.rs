...
mod tests {
    ...
    fn every_receipt_builder_is_bound_to_its_lane_prediction_wrapper() {
        ...
        assert_eq!(
            image_receipts, 8,
            "all eight MLX image receipt builders must use the image policy wrapper \
             (sc-22726 added the FLUX.1 family arm; sc-22731 the SANA/Chroma1 one)"
        );
        assert_eq!(
            source
                .lines()
...
/// The weights-free conformance identities the two engines publish for a registry-behaviour
...
#[cfg(test)]
fn is_sana_chroma_weights_free_fingerprint(fingerprint: &str) -> bool {
...
mod sana_chroma_tests {
    ...
    fn every_planned_sana_chroma_mlx_cell_names_the_identity_its_loaded_generator_publishes() {
        ...
        for (key, entry) in plan["anchors"].as_object().expect("anchors object") {
            ...
            let planned = entry["calibrationFingerprint"]
                ...
                .unwrap_or_else(|| panic!("{key}: calibrationFingerprint must be a string"));
            assert!(
                !is_sana_chroma_weights_free_fingerprint(planned),
                "{key}: {planned} is a weights-free conformance identity no production load returns"
            );
            assert_eq!(
                planned,
        ...
        assert_eq!(
            identities.len(),
            15,
            "two SANA routes and three Chroma1 routes, three shipped tiers each"
        );
    ...
    fn every_planned_mlx_fixture_is_accepted_by_the_arm() {
        ...
        assert_eq!(checked, 15, "fifteen MLX SANA/Chroma1 plan rows");
    }
}
